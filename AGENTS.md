...
- **API tokens** (Settings → API / MCP): Bearer `Authorization` for agents; hashed in `api_tokens`. Streamable HTTP MCP at `/mcp` (same auth gate). Token CRUD requires a browser session (tokens cannot mint tokens). Opt-in `remote_exec` permission gates `exec_command` and setting/changing `preBackupCommands` (session/unlocked always allowed). Opt-in `read_only` (mutually exclusive with `remote_exec`) omits mutating MCP tools from `tools/list` (and still denies at call time) and blocks non-GET `/api/*` except `/mcp` and `validate` / `test_*`; existing tokens without it still write. `exec_command` is only registered when `remote_exec` (or a session) is allowed.
...
| `api_tokens` | Named Bearer tokens (SHA-256 hash + prefix); optional JSON `permissions` (e.g. `["remote_exec"]` or `["read_only"]`); used by MCP / machine clients |
...
9. Docker volume sources need `docker` + `alpine` on the source (SSH server **or** this host’s socket). Destinations are always paths/prefixes (never “to volume”). Restore for path/volume/database requires a **local** artifact or a download from **S3**, Bro, or an **SSH destination with key auth** (password-only dests cannot pull). History can restore onto a different host.
...
13. Bearer tokens without `remote_exec` cannot set/change `preBackupCommands` or call `exec_command` / `POST …/servers/:id/exec`. Browser sessions always can. Existing tokens default to no `remote_exec` after migration — recreate with the checkbox if needed. Bearer `read_only` tokens cannot mutate via MCP or REST (GET + `validate_backup` / `test_*` still allowed; write tools omitted from MCP `tools/list`); empty `[]` permissions still write except `remote_exec`.
