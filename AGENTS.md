...
- **Source types:** `path` (filesystem) or `docker_volume` (named volume on a **source server** only → alpine tar → `.tar.gz` landed at the destination **path**). Destinations are never Docker volumes. Restore pushes a **local** artifact back and extracts into a volume on the source server.
...
src/lib/backup/    # executeBackup, restoreDockerVolumeBackup, file-retention, storage-stats, destination, log-format
...
Backup: resolve From/To → pre-backup cmds → path transfer (pull/push/local/ephemeral|relay) **or** docker pack+land archive → version/file retention → history + artifactPath
Restore (docker, local dest only): history artifact → push → alpine extract into volume
```
...
| `backup_configs` | `sourceKind`/`destinationKind` local\|server, nullable `serverId` (source) + `destinationServerId`, `sourceType` path\|docker_volume, `sourcePath`/`destinationPath`, cron, excludes, pre-cmds, versioning + file retention |
...
5. Temp SSH identity + `-F /dev/null` (ignore host ssh config); ephemeral keys cleaned in `finally`.
6. Cleanup: version count and/or age-based file retention (local FS or remote over SSH).
7. History + `combineBackupLog`; storage stats for local dest (`storage-stats.ts`); remote dest returns a marker.
8. **Restore:** `POST /api/history/[id]/restore` → local artifact → push → extract into volume (local destinations only).
